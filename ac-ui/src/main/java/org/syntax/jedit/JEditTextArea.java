...
public class JEditTextArea extends JComponent {
	...
	public JEditTextArea(TextAreaDefaults defaults) {
		...
		// Manuel Freire: mouse wheel handler
	...
	public int yToLine(int y) {
		...
		return Math
				.max(0, Math.min(getLineCount() - 1, y / height + firstLine));
	}
	...
	public int _offsetToX(int line, int offset) {
		...
		if (tokenMarker == null) {
			...
			return x
					+ Utilities.getTabbedTextWidth(lineSegment, fm, x, painter,
							0);
		}
		 ...
		 * tokens can vary in width */
			...
			if (painter.currentLineIndex == line
				...
				tokens = painter.currentLineTokens = tokenMarker.markTokens(
						lineSegment, line);
			}
			...
			for (;;) {
				...
				if (offset + segmentOffset < lineSegment.offset + length) {
					...
					return x
							+ Utilities.getTabbedTextWidth(lineSegment, fm, x,
									painter, 0);
				} else {
					lineSegment.count = length;
					x += Utilities.getTabbedTextWidth(lineSegment, fm, x,
							painter, 0);
	...
	public int getLineLength(int line) {
		...
		if (lineElement == null)
			...
			return lineElement.getEndOffset() - lineElement.getStartOffset()
					- 1;
	}
	...
	public void setText(String text) {
		try {
			...
			// Manuel Freire - was remove followed by insert
		...
		// Manuel Freire - ugly hack to make sure things get visible
	...
	public void select(int start, int end) {
		...
		if (newStart != selectionStart || newEnd != selectionEnd
			...
			document
					.addUndoableEdit(new CaretUndo(selectionStart, selectionEnd));
		...
		try {
			if (rectSelect) {
				...
				for (int i = selectionStartLine; i <= selectionEndLine; i++) {
					...
					document.insertString(rectStart, selectedText.substring(
							lastNewline, currNewline), null);
	...
	protected void updateBracketHighlight(int newCaretPosition) {
		...
		try {
			int offset = TextUtilities.findMatchingBracket(document,
					newCaretPosition - 1);
			if (offset != -1) {
	...
	protected void documentChanged(DocumentEvent evt) {
		DocumentEvent.ElementChange ch = evt.getChange(document
	...
	class ScrollLayout implements LayoutManager {
		...
		public void layoutContainer(Container parent) {
			...
			right
					.setBounds(ileft + centerWidth, itop, rightWidth,
							centerHeight);
			...
			while (status.hasMoreElements()) {
				...
				comp.setBounds(ileft, itop + centerHeight, dim.width,
						bottomHeight);
				ileft += dim.width;
			}
	...
	/**
	 * Manuel Freire: a simple mouse wheel scroll listener
	 */
	class WheelHandler implements MouseWheelListener {
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
				...
				int next = getFirstLine();
				if (e.getWheelRotation() > 0) {
					// scroll down
					next += getVisibleLines() + n;
					if (next > max)
						next = max;
				} else {
					// scroll up
	...
	class MouseHandler extends MouseAdapter {
		...
		private void doSingleClick(MouseEvent evt, int line, int offset, int dot) {
			...
			// Manuel Freire: repaint to get highlighting right
			repaint();
		}
