...
public abstract class InputHandler extends KeyAdapter {
	...
	public static String getActionName(ActionListener listener) {
		// Manuel Freire: changed variable name 'enum' (now a keyword) to 'actions'
		Enumeration actions = getActions();
		while (actions.hasMoreElements()) {
			String name = (String) actions.nextElement();
	...
	protected void handleGrabAction(KeyEvent evt) {
		...
		grabAction = null;
		executeAction(_grabAction, evt.getSource(), String.valueOf(evt
				.getKeyChar()));
	}
	...
	public static class backspace_word implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			if (caret == 0) {
				...
				String noWordSep = (String) textArea.getDocument().getProperty(
						"noWordSep");
	...
	public static class end implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (caret == lastDocument) {
				textArea.getToolkit().beep();
				return;
			} else if (!Boolean.TRUE.equals(textArea
					.getClientProperty(SMART_HOME_END_PROPERTY)))
				caret = lastOfLine;
	...
	public static class document_end implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			JEditTextArea textArea = getTextArea(evt);
			if (select)
				textArea.select(textArea.getMarkPosition(), textArea
						.getDocumentLength());
			else
	...
	public static class home implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			int firstOfLine = textArea.getLineStartOffset(textArea
					.getCaretLine());
			int firstVisibleLine = (firstLine == 0 ? 0 : firstLine
	...
	public static class next_line implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (magic == -1) {
				magic = textArea.offsetToX(line, caret
	...
	public static class next_page implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			int caret = textArea.getLineStartOffset(Math.min(textArea
					.getLineCount() - 1, line + visibleLines));
			if (select)
				textArea.select(textArea.getMarkPosition(), caret);
			else
				textArea.setCaretPosition(caret);
	...
	public static class prev_page implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			int caret = textArea.getLineStartOffset(Math.max(0, line
					- visibleLines));
			if (select)
				textArea.select(textArea.getMarkPosition(), caret);
			else
	...
	public static class toggle_rect implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			JEditTextArea textArea = getTextArea(evt);
			textArea
					.setSelectionRectangular(!textArea.isSelectionRectangular());
		}
	}
